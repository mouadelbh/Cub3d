...
/*   Updated: 2025/02/27 17:55:06 by mel-bouh         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	*put_pixel_color_colm(t_mlx *c, t_imge *txt)
{
	...
	{
		tmp[i] = find_txt_pixel_color(txt, (int)c->draw.txt_x, (int)(y
					* scale));
		y++;
...
int	put_pixel_col(t_mlx *c, t_imge *txt, int x)
{
	...
	while (++i < c->draw.draw_start)
		my_mlx_pixel_put(&c->img, x, i, find_color(c->draw.c[0], c->draw.c[1],
				c->draw.c[2]));
	j = 0;
